...
|---|---|---|---|---|---|---|---|---|---|
| r | Sin textos repetidos | # de repeticiones en pantalla (overlay ≈ subtítulo hablado, overlays solapados, títulos duplicados) | **`python/check_text_overlap.py`** (Jaccard de tokens vs subtítulo, sim≥0.6) | **31 → 1** *(loop 4: fix = suprimir baseline durante tarjeta editorial con texto)* → **nivel Excelente** ✅. Residual: 1 caso card.subtitle+dataViz (borde, ⚠️ para llegar a 0) | ≤10 | **≤3 ✅** | **0** | 8 | 4/5/9 |
